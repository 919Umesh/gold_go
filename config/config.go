...
type Config struct {
	...
	WorkerCount  int
	QueueSize    int
	DatabaseURL  string // Add this new field
}
...
func InitConfig() *Config {
	configOnce.Do(func() {
		// First, try to get DATABASE_URL from environment (Render provides this)
		databaseURL := os.Getenv("DATABASE_URL")

		if databaseURL != "" {
			// If DATABASE_URL is provided (like in Render), use it
			configInstance = &Config{
			...
			configInstance = &Config{
				...
				WorkerCount:  getEnvAsInt("WORKER_COUNT", 5),
				QueueSize:    getEnvAsInt("QUEUE_SIZE", 100),
				DatabaseURL:  "", // Empty for local development
			}
		}
	})
	return configInstance
